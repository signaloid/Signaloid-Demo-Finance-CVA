...
Ensures that compiler and linker diagnostics are forwarded to `stderr` in a clean, readable form when the build fails, so that the output can be consumed by the SCCE backend or other tooling.
...
Redirecting to `stderr` is intentional: the SCCE backend reads build diagnostics from `stderr`, not `stdout`.
