...
**Short-rate model.** The interest-rate environment follows the Hull-White / GSR one-factor model with constant mean reversion `a = 0.02`, constant volatility `σ = 0.0075`, and a flat 3% initial curve. The auxiliary Ornstein-Uhlenbeck state `x(t)` and the analytic zero-coupon bond are

```
dx = -a x dt + σ dW,     r(t) = x(t) + α(t)
P(t, T) = A(t, T) · exp(-B(t, T) · r(t))
B(t, T) = (1 - e^{-a(T - t)}) / a
```

**Hazard curve.** A backward-flat counterparty hazard-rate curve with pillars `λᵢ = 0.02 · i` at integer years, giving the survival probability `S(t) = exp(-∫₀ᵗ λ(s) ds)`.

**CVA.** With recovery `R = 0.4` and discount factor `DF(t) = e^{-r₀ t}`, the unilateral CVA is

```
CVA = (1 - R) · Σₙ E[max(V(tₙ, x_tₙ), 0)] · DF(tₙ) · [S(tₙ₋₁) - S(tₙ)]
```
...
- **Credit Valuation Adjustment (`-S 0`)**: the unilateral CVA of the netting set, reported in currency units. The Hull-White volatility `σ` is treated as the uncertain model input (default `Gauss(0.0075, 0.0005)`), and the CVA is reported as the expectation of the resulting exposure distribution — a single scalar value.
...
This runs 1,500 Monte Carlo iterations to compute the CVA with the Hull-White volatility fixed at `σ = 0.0075` and prints the kernel timing. The converged CVA mean is written to `data.out`, where the first line contains the execution time in microseconds (μs) and the following line contains the output value.
...
| Short | Long | Type | Default | Description |
|-------|------|------|---------|-------------|
| `-x` | `--volatility` | double | `Gauss(0.0075, 0.0005)` | Hull-White short-rate volatility `σ`. When omitted, `σ` is treated as an uncertain (Gaussian) input. When set, `σ` is fixed to the given value. |
